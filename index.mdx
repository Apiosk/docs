...
The gateway acts as a public x402 payment boundary. A client requests execution, receives pricing in a `402 Payment Required` response when the route is paid, settles in USDC on a supported network, Base, Polygon, or Arbitrum, retries with a proof, and then receives the upstream result through the same public surface. See [Multi-chain Settlement](/guides/multichain).
...
Three modes, and the difference is where the traffic goes.

| Mode | Your API is served from | `402` issued by | Guide |
| --- | --- | --- | --- |
| **Gateway** | `gateway.apiosk.com/{slug}/…`, proxied to your origin | Apiosk | [/guides/publishers](/guides/publishers) |
| **SDK** | **your own URL, unchanged** | the Apiosk middleware, on your host | [/guides/sdk-mode](/guides/sdk-mode) |
| **Federated** | your own URL | your own code, from your `/.well-known/x402` | [/guides/sdk-mode](/guides/sdk-mode) |

SDK mode is the middle path: you keep your traffic and your existing customers,
Apiosk keeps discovery, pricing, verification, analytics and payouts. Install the
middleware in front of the routes you already run
([/guides/sdk-middleware](/guides/sdk-middleware)), or implement the same two
HTTP calls yourself ([/guides/integrations-api](/guides/integrations-api)).

If your buyer is a **person with a browser** rather than an agent, use the hosted
checkout at `pay.apiosk.com`: you create a checkout server-to-server, redirect the
buyer, and their own wallet pays your seller, you, and Apiosk in one transaction.
See [Apiosk Pay](/pay/overview).
