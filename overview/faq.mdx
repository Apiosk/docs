...
On the USDC rail, Apiosk takes a flat **2%** platform fee at the moment of settlement on Base — publishers receive 98% of each call. The split is enforced by the on-chain `ApioskSettlement` contract, so it is verifiable from any block explorer. The fee was reduced from 10% to 2% so that publishers keep substantially more of each call.
