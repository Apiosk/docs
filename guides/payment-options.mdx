...
If you are using the local Apiosk MCP package, pass that connect string to the package and check it with `apiosk_connect`, which reports the wallet, policy and limits the session may use.
...
- for local MCP installs, start with `apiosk_connect` and let it tell you whether you are ready or still need funding
