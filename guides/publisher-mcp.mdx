...
provider portal under **Settings → SDK & API keys**, then add the server to your MCP
client:
...
`401` telling you how to get one. For local stdio installs of the npm package `@apiosk/mcp` (binary `apiosk-mcp-server`),
...
Pricing is per call in USDC. Base is the default settlement network; only
`base`, `polygon`, and `arbitrum` actually settle, and they follow the
[multi-chain settlement](/guides/multichain) rules (Base always remains the
fallback). `solana` is still accepted by the tool's input validation but no
longer resolves to a USDC token: the publisher tool logs a warning and silently
falls back to Base, so do not request it. Your `settlement_address` receives 98% of every paid call.
