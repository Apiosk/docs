...
If you are integrating through the Apiosk MCP package in local stdio mode, run `apiosk_connect` first. It reports whether this session can buy, which wallet and limits apply, and returns a connection link when there is none.
...
If you are using the local MCP package, you normally do not need to build this first step by hand. `apiosk_connect` tells you whether the session can buy and what still needs funding.
