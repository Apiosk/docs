...
| --- | --- | --- | --- |
| Base | `base` | `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913` | **Live, always on** |
| Polygon | `polygon` | `0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359` | Live when the facilitator supports it |
| Arbitrum | `arbitrum` | `0xaf88d065e77c8cC2239327C5EDb3A432268e5831` | Live when the facilitator supports it |
...
4. Put the network name (`base`, `polygon`, `arbitrum`) in the payment
...
- attach **up to 3 payout wallets per API: exactly one per network**, chosen
  from your verified wallets
- the connected wallet's chain is auto-detected from the wallet's active chain:
  Base, Polygon, or Arbitrum
- wallet ownership must be verified before a wallet can be attached: the wallet
  signs the ownership message with an EIP-191 signature
- `solana` is not an accepted value: the payout wallet RPC raises
  `unsupported payout network` for anything outside base, polygon, and arbitrum

## Payout routing

Earnings settle to the wallet of the chain the buyer paid on: pay on Polygon,
and the publisher's Polygon wallet receives the payout; pay on Base, and the
