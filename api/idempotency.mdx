...
Keys are scoped to the calling API key and the logical operation. Two keys,
or two different source operations, can use the same string without
colliding. The v1 and v2 direct-call path aliases identify the same logical
operation.
...
| `POST /v2/asks/{id}/continue` | Requires `Idempotency-Key`; same state/action/input resumes the same run. `limit` and `wait_seconds` may change. |

Continuations without a key are refused. Their keys are scoped to the calling
key and task. Keep the original signed state when retrying the same choice;
use a new idempotency key for a newly reviewed action.
