...
Every refusal has the same shape. Provider errors never reach you raw.
...
- **Anything that refuses before a purchase says "Nothing was charged."** That covers
  every `4xx` above except `502`/`504`, which arrive after a source was called.
- **A failed run** returns `502` with the run body: `usage` shows what was
  charged, and anything undelivered was refunded or is held until reconciled.
- **When in doubt, read the run.** `GET /v2/runs/{id}` is free and authoritative.
