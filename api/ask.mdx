...
details for current prices. Planning is free. <code>estimated_total</code> is
...
A <code>needs_input</code> response may contain field schemas, a natural
language <code>message</code>, or both. Ask the person for the missing
context; never fill it in by guessing. A <code>needs_selection</code> response
offers candidates, so ask which entity they meant.

The buyer HTTP API does not expose the App/MCP task-continuation action:
submit a new clarified question with a new <code>Idempotency-Key</code>.
Planning remains free. Keep the earlier ask id for its audit trail.
